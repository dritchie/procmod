...
return S.memoize(function(makeGeoPrim, geoRes)
	...
	local function genBoxWingSeg(xbase, zlo, zhi)
		local zbase = uniform(zlo, zhi, "boxwingzbase")
		local xlen = uniform(0.25, 2.0, "boxwingxlen")
		local ylen = uniform(0.25, 1.25, "boxwingylen")
		local zlen = uniform(0.5, 4.0, "boxwingzlen")
		wingseg(xbase, zbase, xlen, ylen, zlen)
		if flip(0.5, "genguns") then
			local gunlen = uniform(1.0, 1.2, "gunlen")*zlen
	...
	local function genCylWingSeg(xbase, zlo, zhi)
		local zbase = uniform(zlo, zhi, "cylwingzbase")
		local radius = uniform(0.15, 0.7, "cylwingradius")
		local xlen = 2*radius
		local ylen = 2*radius
		local zlen = uniform(1.0, 5.0, "cylwingzlen")
		wingcyls(xbase, zbase, radius, zlen)
		return xlen, ylen, zlen, zbase
	end

	local function genWingSeg(xbase, zlo, zhi)
		local segType = multinomial(wingSegTypeWeights, "wingsegtype")
	...
	local function genBoxBodySeg(rearz, prev, taper)
		local xlen = uniform(1.0, 3.0, "boxxlen")
		local ylen = uniform(0.5, 1.0, "boxylen") * xlen
		local zlen
		...
		if taper then
			zlen = uniform(1.0, 3.0, "taperboxzlen")
			local taper = uniform(0.25, 0.75, "taperamt")
			taperedbox(0.0, 0.0, rearz + 0.5*zlen, xlen, ylen, zlen, taper)
		else
			zlen = uniform(2.0, 5.0, "boxzlen")
	...
	local function genCylinderBodySeg(rearz, prev)
		...
		local radius = uniform(minrad, maxrad, "bodycylrad")
		local xlen = radius*2
		local ylen = radius*2
		local zlen = uniform(2.0, 5.0, "bodycyllen")
	...
	local function genCylClusterBodySeg(rearz, prev)
		...
		local radius = uniform(minrad, maxrad, "bodycyclusterrad")
		local xlen = radius*4
		local ylen = radius*4
		local zlen = uniform(2.0, 5.0, "bodycylclusterlen")
	...
	local function genBodySeg(rearz, prev)
		...
		local ylen
		local zlen
		local segType = multinomial(segTypeWeights, "bodysegtype")
