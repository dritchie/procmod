...
local Camera = S.memoize(function(real)
	...
	Camera.methods.__init = terralib.overloadedfunction('Camera.init', {
		terra(self: &Camera, eye: Vec3, target: Vec3, up: Vec3, worldUp: Vec3, fovy: real, aspect: real, znear: real, zfar: real) : {}
			self.eye = eye
			self.target = target
			self.up = up
			self.absUp = worldUp
	...
	Camera.methods.__init:adddefinition(
		terra(self: &Camera, eye_x: real, eye_y: real, eye_z: real, target_x: real, target_y: real, target_z: real, up_x: real, up_y: real, up_z: real,
...
local Light = S.memoize(function(real)
	...
	Light.methods.__init = terralib.overloadedfunction('Light.__init', {
		terra(self: &Light)
		...
		terra(self: &Light, type: LightType, posOrDir: Vec3, ambient: Color4, diffuse: Color4, specular: Color4) : {}
			self.type = type
			self.pos = posOrDir
		...
		terra(self: &Light, type: LightType, posOrDir: Vec3, diffuse: Color4, ambAmount: real, specular: Color4) : {}
			self.type = type
			self.pos = posOrDir
...
local Material = S.memoize(function(real)
	...
	Material.methods.__init = terralib.overloadedfunction('Material.__init', {
		terra(self: &Material)
		...
		terra(self: &Material, ambient: Color4, diffuse: Color4, specular: Color4, shininess: real)
			self.ambient = ambient
			self.diffuse = diffuse
			self.specular = specular
			self.shininess = shininess
		end,
		terra(self: &Material, diffuse: Color4, specular: Color4, shininess: real)
