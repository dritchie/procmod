...
local mhsmc = terralib.require("mhsmc.mhsmc")
...
local N_GENERATIONS = 20
local RECORD_HISTORY = true
local run = mhsmc.run(p)
return terra(generations: &S.Vector(S.Vector(qs.SampleType(p))))
	generations:clear()
	run(N_PARTICLES, N_GENERATIONS, generations, RECORD_HISTORY, true)
end
