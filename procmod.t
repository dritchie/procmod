				...
				if doShadowMatch then
					...
					emit quote
						if self.score ~= [-math.huge] then
							...
							-- -- TEST: verify that shadow map looks ok
							...
							-- end
							-- S.free(depthdata)
							-- dimg:save(image.Format.TIFF, "shadowMapTest.tiff")
							...
							gl.glFlush()
							-- Compare
							self.shadowMatchImagePixelData:resize(w*h)
							gl.glReadPixels(0, 0, w, h, gl.GL_RGBA, gl.GL_UNSIGNED_BYTE, &(self.shadowMatchImagePixelData(0)))
							self.shadowMatchImage:clear()
							self.shadowMatchImage:resize(h, w)
							-- -- TEST: verify that the result looks ok
							-- var img = [image.Image(uint8, 4)].salloc():init(w, h)
							for y=0,h do
								for x=0,w do
									var c = self.shadowMatchImagePixelData(y*w + x)
									if c(0) > 0 then
										self.shadowMatchImage:setPixel(y, x)
									end
									-- img(x,y) = c
								end
							end
							-- img:save(image.Format.PNG, "shadowRenderTest.png")
