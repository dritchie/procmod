...
-- Constants
local outfilename = arg[2] or "experiments/pgibbsSweeps.csv"
local nTotal = 300
local sweepNums = {}
for n=1,15 do table.insert(sweepNums, n) end
local numRuns = 20
...
local terra ogl_main()
	...
	gl.glutCreateWindow("Particle Gibbs Num Sweeps Experiment")
end
ogl_main()

-- Where the work happens: run for a given sweep num, record timing and score info
local function dowork(nSweeps, record)
	print(string.format("   nSweeps = %d", nSweeps))
	globals.config.pgibbs_nSweeps = nSweeps
	globals.config.nSamples = math.floor(nTotal / nSweeps)
	...
	if record then
		...
		f:write(string.format("%d,%g,%g,%g,%g\n",
			nSweeps, t1-t0, avgscore, varscore, maxscore))
		f:flush()
	end
end
...
for _,nSweeps in ipairs(sweepNums) do
	for i=1,numRuns do
		dowork(nSweeps, true)
	end
end
